...
class IMSApp {
    ...
    handleDelegatedFormSubmit(event) {
        ...
        if (form.id === 'predictionForm') {
            event.preventDefault();
            event.stopPropagation();
            this.handlePredictionForm(event);
    ...
    async handlePredictionForm(event) {
        ...
        // Support both naming conventions: quantity1/2/3 and period1/2/3
        const quantity1 = this.parseFloatSafe(
            formData.get('quantity1') ?? formData.get('period1')
        );
        const quantity2 = this.parseFloatSafe(
            formData.get('quantity2') ?? formData.get('period2')
        );
        const quantity3 = this.parseFloatSafe(
            formData.get('quantity3') ?? formData.get('period3')
        );

        const data = { quantity1, quantity2, quantity3 };
        ...
        try {
            if (submitBtn) this.setButtonLoading(submitBtn, 'Predicting...');
            this.showLoading(form);
            const result = await this.makePrediction(data);
            ...
            this.hideLoading(form);
            if (submitBtn) this.resetButton(submitBtn, originalText);
        }
    }
